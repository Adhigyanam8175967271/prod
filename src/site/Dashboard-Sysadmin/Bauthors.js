...
const handleSubmit = (event) => {
  ...
  axios.post('https://adhigyanam-e92bf1bbbdb1.herokuapp.com/uploadbauthor', formData)
    .then(res => {
      if (res.data === "Success") {
...
const handleDelete = (clientId) => {
  ...
  axios.delete(`https://adhigyanam-e92bf1bbbdb1.herokuapp.com/deletebauthor/${clientId}`)
    .then(response => {
    ...
    return (
      ...
      <p className="mb-12 text-secondary-light" style={{fontSize:"15px"}}>
                                   <b>You are here</b>: <NavLink to="/dashboard" style={{color:'blueviolet', textDecoration:"underline"}}>Dashboard</NavLink> | Blog Authors
                               </p>
      ...
      <div className="auth-left py-32 px-24 d-flex flex-column" style={{background:"none",width:"auto"}}>
                    ...
                    <div>
                               ...
                               <p className="mb-6" style={{fontWeight:'bold',fontSize:"18px"}}>A. Create New Author</p>
                               <p className="mb-32 text-secondary-light text-lg">
                                   Use the following form to add a new blog author to the website.<br/> <b>Recommended Resolution: 500 X 500</b>
                               </p>
            ...
            />
             {errorsfield.fdesc && <span className="text-danger" style={{ fontSize: "0.8rem", fontWeight: "bolder" }}>{errorsfield.fdesc}</span>}
                    ...
                    <div>
                                        ...
                                        {isSubmitting ? 'Please Wait...' : 'Create Author Profile'}
                                        </Button>
                   ...
                   <div className="auth-right py-32 px-24 d-flex flex-column">
                       <div className="max-w-464-px mx-auto w-100">
                           ...
                           <p className="mb-6" style={{fontWeight:'bold',fontSize:"18px"}}>B. Manage Existing Authors</p>
                               <p className="mb-32 text-secondary-light text-lg">
                                   Use the following modify / remove listed authors.
                               </p>
  ...
  <tbody>
    {clients.map(client => (
      <tr key={client.Id} style={{ borderBottom: "1px solid #ddd", backgroundColor: "#f9f9f9", transition: "0.3s" }}>
        ...
        <td style={{ padding: "10px", fontSize: "0.9rem", color: "#333", backgroundColor:"white" }}><b>{client.Aname}</b><br/>{client.Atitle}</td>
